...
(function(win){
  ...
  function searchAjax(pageIndex, personOnly){
    ...
    $.ajax({
      url: 'http://localhost:3004/list21',
      type: 'GET',
      dataType: 'json',
      data: {
        keyWords: configAll.keyWords,
        departEnable: configAll.opts.departEnable,
        personEnable: configAll.opts.personEnable,
  ...
  function select(data){
    ...
    if (configAll.opts.selectType == 'radio'){
      if ($cd(configAll.$selectDepartList).length) {
        var datas = $cd(configAll.$selectDepartList)[0].contact;
        $cd(configAll.$selectDepartList)[0].parentNode.removeChild($cd(configAll.$selectDepartList)[0]);
        unselect(datas);
      };
      if($cd(configAll.$selectStaffList).length) {
        var datas = $cd(configAll.$selectStaffList)[0].contact;
      ...
      if (el) {
        ...
        if (configAll.departSelected == false) {
          for (var i = 0; i < childrens.length; i++) {
            if (childrens[i].contact.id != configAll.opts.myDepartmentId) childrens[i].disabled = false;  //我的部门除外
            };
          if (configAll.opts.personEnable != 'off') {
