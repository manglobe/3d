...
win.layer = {
  ...
  msg:function(content,options){
    ...
    this.open({
      content: content,
      className:'wj-msg',
      shade: options.shade == undefined ? true : options.shade,
      time: options.time || 2,
    });
  },
  ...
  alert:function(content,options){
    if(options==null){
      ...
      this.open({
        title: [
          options.title,
          'height:46px;background-color:#ffffff;margin:0;font-size:16px; color:#333333;line-height:46px;text-align:center;'
        ],
        content: content,
        btn: [options.btn||"确定"],
        className:'wj-alert',
  ...
  //无按钮
  alert01:function(content,title){
    this.open({
      title: [
        title || '无标题',
        'height:46px;background-color:#ffffff;margin:0 10px !important;padding:0 !important;font-size:16px; color:#333333;line-height:46px;text-align:center;'
      ],
      content: content,
      className:'wj-alert01',
      shadeClose: false,
    });
  },
  ...
  prompt:function(content,options){
	...
	var style='';
	if(options.title){
		style='height:46px;border:none;margin:0 !important;padding:20px 10px 0 !important;font-size:16px; color:#333333;line-height:24px;text-align:center; background:rgba(248,248,248,0.80);border-top-left-radius:8px; border-top-right-radius:8px;';
	}else{
		style='display:none;';
	}
    this.open({
      className:'wj-prompt',
      title: [options.title, style],
      content:content,
      btn: [
		options.yes || "确定",
		options.no || "取消"
    ...
    setTimeout(function(){
      document.querySelector(".wj-prompt .layermend").style.display = 'none';
    },10);
  },
  customDialog:function(id,title){
    this.open({
      type:1,
      className:'wj-custom',
      title: [
        title,
        'height:46px;margin:0 10px !important;padding:0 !important;font-size:16px; color:#333333;line-height:46px;text-align:center;'
      ],
      shadeClose: false,
      content:$('#'+id).html(),
      fixed:true,
...
'function' == typeof define ? define(function() {
  ...
  document.head.appendChild(function(){
    var link = doc.createElement('link');
    link.href = path + 'need/layer.ext.css';
    link.type = 'text/css';
    link.rel = 'styleSheet'
    link.id = 'layerextcss';
    return link;
